...
## A1 — LEVEL 1 : PREMIER CONTACT (à envoyer par toi)

### Message (identique pour les deux fournisseurs, EN)

```
Subject: Wholesale/Dropship Inquiry — Custom Engraved Necklace (Test Order)

Hello,

We are currently evaluating suppliers for an initial market test in France
for a custom engraved necklace product. Before discussing volume, we'd like
to confirm product fit and unit economics with a single test order.

Could you please share:

1. Unit price for ONE custom-engraved necklace (single-piece / dropship test scenario)
2. Exact base material composition
3. Type of gold plating (process + thickness if available)
4. Engraving cost (per piece)
5. Packaging cost (per piece)
6. Total landed cost / DDP price for ONE order shipped directly to an
   end customer in France (all fees included)
7. Timelines:
   - personalization / production time
   - dispatch time
   - delivery time to France
8. Can you do personalized dropshipping starting from 1 unit (no MOQ)?
9. Can shipments be fully white-label (no branding, no invoice, no
   supplier info visible to the end customer)?
10. Do you support Shopify / API / CSV or any other order-automation
    integration?
11. Total price for ONE personalized sample delivered to us in France
    (product + engraving + packaging + shipping, all-in)
12. Total time from sample order to delivery in France

We'd also appreciate:
- real product photos
- macro photos of the engraving
- photos of the chain/clasp
- packaging photos
- a real product video if available

We are currently evaluating suppliers for an initial market test in
France. If the product meets our requirements, we will then discuss
volume pricing and a longer-term partnership.

Thank you,
[Name] — Verlyne
```

Note : je n'ai PAS mentionné de volume, de chiffres de ventes ou d'engagement de 1000 unités, conformément à la consigne.

### Coordonnées disponibles (via recherche web — non vérifiées par contact direct)
...
| ShineOn | Email : hello@shineon.com · App Shopify installée : https://sellers.shineon.com/ | App installée le 02/09/2026, catalogue consulté directement par le fondateur. **STATUT : REJECT — style du catalogue incompatible avec le brief de marque** (voir A2-bis). Ne pas relancer. |
...
### Procédure d'envoi exacte
1. Ouvre le lien de contact du fournisseur (ou son profil Alibaba).
2. Colle le message EN ci-dessus tel quel (ou via le chat Alibaba si disponible).
3. Note la date d'envoi et le canal utilisé dans le tableau `RESPONSES` ci-dessous.
4. Quand une réponse arrive, colle-la moi telle quelle (texte + photos si possible) → je fais la classification PASS / NEED MORE INFO / REJECT (A2) et je calcule les coûts (Track B) dès que les chiffres sont exploitables.
...
## A2 — CLASSIFICATION AU RETOUR (Level 1 → décision)

Dès qu'une réponse Level 1 arrive, je l'évalue selon cette grille :

- **PASS** → produit adapté + personnalisation confirmée + expédition France possible + coût potentiellement viable + white-label confirmé + délais raisonnables (production+livraison ≤ ~3 semaines pour un test). → déclenche l'envoi du questionnaire Level 2.
...
### A2-bis — ShineOn = REJECT (02/09/2026)

Signal réel, pas une supposition : l'app ShineOn a été installée sur la boutique (confirmé via `appInstallations`), et **toi-même** as ouvert son catalogue produit réel et jugé le style incompatible avec le brief Verlyne ("minimaliste, premium-accessible, féminin, élégant"). C'est exactement la même logique de validation que pour 96Jewelry — un vrai signal observé, pas une intuition non vérifiée.

- **Motif de rejet :** style catalogue ne correspond pas au positionnement de marque.
- **Ce qui reste vrai malgré le rejet produit :** l'app elle-même (modèle POD, usine UE à Amsterdam, intégration Shopify native, commission uniquement à la vente) n'est pas disqualifiée en tant qu'infrastructure — seul son catalogue actuel ne convient pas. Elle pourrait redevenir pertinente si un jour un besoin POD ponctuel apparaît avec un design différent, mais **elle sort de la comparaison fournisseur active dès maintenant.**
- **Aucune relance envoyée.** Retiré de A4.1/A5/A6 ci-dessous.

---

## SUPPLIER DUE DILIGENCE — LEVEL 2

**Ne pas envoyer avant classification PASS au Level 1.**

Reconstruit à partir des sujets que tu as listés (l'historique du document à 30 questions original n'est pas présent dans ce dépôt — si tu as le fichier source, envoie-le-moi et je le fusionne avec cette version). Structure en 6 blocs, ~30 questions :

### 1. Matière & sécurité (Q1–6)
1. Confirmez-vous que le matériau est du 316L stainless steel (ou précisez la nuance exacte utilisée) ?
2. Quelle est la composition exacte de l'alliage (%) ?
3. Le placage est-il PVD ? Si oui, quelle épaisseur (microns) et quel process (gold-ion, 18K PVD, etc.) ?
4. Avez-vous un certificat de test nickel release (EN 1811) ou équivalent ?
5. Quelle est la résistance testée à la corrosion / à l'eau / à la transpiration ?
6. Le produit est-il conforme REACH (UE) ?

### 2. MOQ & pricing par palier (Q7–11)
7. Quel est le prix unitaire à 1 / 50 / 100 / 500 / 1000 unités ?
8. Le MOQ dropshipping (1 unité) reste-t-il disponible en continu ou seulement en phase de test ?
9. Y a-t-il une remise à partir d'un certain volume mensuel régulier (plutôt qu'une commande unique) ?
10. Le prix inclut-il l'emballage individuel et la gravure, ou sont-ils facturés à part à volume ?
11. Quels sont les délais de paiement / méthodes acceptées (PayPal, virement, carte, Alibaba Trade Assurance) ?

### 3. Branding & packaging (Q12–15)
12. Pouvez-vous produire un packaging custom (boîte brandée Verlyne) à partir de quel volume ?
13. Quel est le coût et le délai de développement d'un packaging custom ?
14. Le colis extérieur (carton d'expédition) peut-il rester totalement neutre (sans logo fournisseur) ?
15. La facture/douane à l'intérieur du colis peut-elle être neutre ou personnalisée à notre nom ?

### 4. Qualité & gestion des erreurs (Q16–21)
16. Quelle est votre politique en cas de défaut produit (pièce cassée, oxydée, mal finie) ?
17. Quelle est votre politique en cas d'erreur de gravure (texte mal gravé, faute) ?
18. Quel est votre taux de défaut moyen constaté sur ce produit ?
19. Que se passe-t-il en cas de colis perdu en transit vers la France ?
20. Proposez-vous un remplacement gratuit ou un remboursement en cas de litige ?
21. Quel est le délai moyen de résolution d'un litige ?

### 5. Capacité & saisonnalité (Q22–24)
22. Quelle est votre capacité de production actuelle par semaine/mois ?
23. Comment évolue votre capacité/délai en période Q4 (novembre-décembre, pic de commandes cadeaux) ?
24. Avez-vous déjà géré un pic de commandes personnalisées pendant la période des fêtes ? Avec quel délai moyen observé ?

### 6. Intégration & opérations (Q25–30)
25. Proposez-vous une intégration Shopify (app, plugin) ou uniquement API/CSV manuel ?
26. Si API : avez-vous une documentation publique ou un contact technique dédié ?
27. Le tracking colis est-il fourni automatiquement (numéro + lien transporteur) pour chaque commande ?
28. Quel transporteur utilisez-vous pour la France, et le tracking est-il fiable de bout en bout (pas seulement jusqu'à la sortie de Chine) ?
29. Acceptez-vous les retours ? Sous quelles conditions (produit personnalisé = généralement non retournable — à confirmer) ?
30. Avez-vous d'autres marques D2C (France/UE) qui travaillent déjà avec vous sur ce type de produit, à titre de référence ?
...
```
SUPPLIER: DG Jewelry
CHANNEL: Formulaire de contact (dgjewelry.cn/contact-us) ou chat Alibaba
MESSAGE: Voir Level 1 ci-dessus (prêt, inchangé)
COST: 0€
ENGAGEMENT: Aucun — simple demande d'information
RECOMMENDATION: Envoie ce message toi-même via le formulaire/Alibaba (je n'ai pas d'accès sortant vers ce domaine ni d'outil d'envoi). Colle-moi la réponse ici dès réception.

SUPPLIER: 96Jewelry
CHANNEL: 96jewelry.com (formulaire à localiser sur le site) ou chat Alibaba
MESSAGE: Voir Level 1 ci-dessus (prêt, inchangé)
COST: 0€
ENGAGEMENT: Aucun — simple demande d'information
...
~~ShineOn~~ — app installée le 02/09/2026, catalogue consulté directement par le fondateur, **REJECT confirmé** (style incompatible avec le brief de marque — voir A2-bis). Retiré des candidats actifs.

Stratégie en 2 temps : un message court de qualification (6 points bloquants) d'abord, le questionnaire complet seulement si la réponse est positive. Réduit la friction, filtre vite les REJECT (comme 96Jewelry en Level 1, et ShineOn en Level 3).
...
### A4.1 — ÉTAPE 1 : message de qualification (version courte, EN) — pour Merryshine, NihaoJewelry
...
### STOP AVANT ENVOI
```
ACTION: Aucun envoi en attente côté nouveaux candidats — Merryshine et NihaoJewelry déjà envoyés le 02/09, réponses en attente. ShineOn REJECT, ne rien envoyer.
COST: 0€ | ENGAGEMENT: aucun
RECOMMENDATION: Dès qu'une réponse Merryshine ou NihaoJewelry arrive, colle-la ci-dessous → je classe PASS/NEED MORE INFO/REJECT et je mets à jour A5.
```
...
**ShineOn — REJECT (02/09/2026), catalogue jugé incompatible avec le brief de marque par le fondateur. Retiré de la comparaison, aucune relance.**
...
*(à remplir uniquement une fois les réponses réelles collées ci-dessus — aucune estimation ne sera substituée à une vraie donnée. ShineOn exclu — voir A2-bis.)*
